...
from .forms import UpdateUserForm, UpdateQuestionnaire, BulkReg, EvalSchedForm, EvalSchedFormOngoing, UpdateInstructorQuestionnaire
from .models import Questionnaire, InstructorQuestionnaire, Student, Instructor, EvaluatedDetail, QuestionnaireScore, Comment, Subject, EvalSched, InstructorQuestionnaireScore
...
def render_pdf_view(request):
  ...
  stud_eval = QuestionnaireScore.objects.all()
  stud_questions = Questionnaire.objects.all()
  inst_name = []
  summary = []
  for inst in instructor:
    question_summary=[]
    for question in stud_questions:
      question_score = 0
      question_score_total = 0
      for eval in stud_eval:
        if inst.user.id == eval.evaluated.user.id and question == eval.question:
          question_score += eval.score
          question_score_total += 1
          print(eval.score)
      if question_score_total == 0:
        ave = 'N/A'
      else:
        average = question_score/question_score_total
        ave = "{:.2f}".format(average)
      question_summary.append({'instructor': inst.user.first_name + " " + inst.user.last_name,'category': question.category, 'question': question.question, 'average': ave})
    inst_name.append(inst.user.first_name + " " + inst.user.last_name)
    summary.append(question_summary)
  template_path = 'main/print_allresponses.html'
  context = {'instructor':inst_name, 'summary': summary}
  response = HttpResponse(content_type='application/pdf')
  # response['Content-Disposition'] = 'attachment; filename="report.pdf"'
  response['Content-Disposition'] = 'filename="Students_Evaluation(Summary).pdf"'
...
def evalsched(request):
  ...
  if len(evalsched_check) == 0:
    if request.method == 'POST':
      ...
      if form.is_valid():
        form.save()
        messages.success(request, 'Evaluation Schedule set')
...
def irreg_sign_up_subjects(request, sections):
  ...
  irreg_subjects = []
  for sec in section:
    get_subjects = EvaluatedDetail.objects.filter(section=sec)
      ...
      if 'first_name' and 'last_name' and 'student_number' and 'section' in df.columns:
        ...
        if missing:
          ...
          for data in df.values:
            ...
            user_check = User.objects.filter(username=student_no)
            skip = False
            if len(user_check) != 0:
              skip = True
      ...
      if 'first_name' and 'last_name' and 'email' and 'department' and 'role' in df.columns:
        ...
        if missing:
          ...
          for data in df.values:
            ...
            if skip == True:
              username_found.append(first_name+' '+last_name)
              continue

            user_instructor = User(
              first_name = first_name,
              last_name = last_name,
              username = username,
              email = email,
          ...
          if len(username_found) != 0:
            for found in username_found:
              message = 'User already registered: ' + found
              messages.warning(request, message)
      else:
        messages.warning(request, 'File is not a register file')
        return redirect('/sign-up/%s' % role)
...
def evaluation_select(request):
  if request.user.is_superuser or (request.user.role == 'Instructor' and request.user.instructor.access_lvl == 3):
    return render(
      request,
    ...
    to_eval = []
    if request.user.role == "Student":
      check_completed = QuestionnaireScore.objects.filter(author_id=request.user.id)
      get_completed = []
      if len(check_completed) != 0:
        for check in check_completed:
          if not get_completed:
            get_completed.append(check.evaluated.user.id)
          else:
            for get in get_completed:
              if check.evaluated.user.id != get:
                get_completed.append(check.evaluated.user.id)
      to_evaluate = EvaluatedDetail.objects.all()
      ...
      check_completed = InstructorQuestionnaireScore.objects.filter(author_id=request.user.id)
      get_completed = []
      if len(check_completed) != 0:
        for check in check_completed:
          if not get_completed:
...
def questionnaire(request, evaluated):
  ...
  check_completed_inst = InstructorQuestionnaireScore.objects.all()
  check_sub = EvaluatedDetail.objects.all()
  check_dept = Instructor.objects.all()
  get_evaluated = Instructor.objects.get(user_id=evaluated)
  ...
  if request.method == 'POST':
    ...
    if comment_sentiment['compound'] > 0.05:
      cs = 'Positive'
    elif comment_sentiment['compound']  > -0.05 and comment_sentiment['compound']  < 0.05:
      cs = 'Neutral'
    elif comment_sentiment['compound']  < -0.05:
      cs = 'Negative'

    sentiment_score = 'positive: ' + str(comment_sentiment['pos'] * 100) + '% neutral: ' + str(comment_sentiment['neu'] * 100) + '% negative: ' + str(comment_sentiment['neg'] * 100) + '%'

    if (cs == 'Positive' and neutral_score > score_comment_checker) or (cs == 'Negative' and neutral_score < score_comment_checker):
      is_connected = False
    else:
      is_connected = True
    ...
    for score in get_question_score:
      if request.user.role == 'Student':
        ...
        instruct_question_score = InstructorQuestionnaireScore(
          score = score['score'],
          author_id = score['author'],
          question_id = score['question'],
          evaluated_id = score['evaluated']
        )
        instruct_question_score.save()
  ...
  if request.user.role == 'Student':
    ...
    for check in check_completed_inst:
      if check.author.id == request.user.id and check.evaluated.user.id == evaluated:
        if request.user.instructor.access_lvl != 3:
          return redirect('/evaluation')
        else:
  ...
  if request.user.role == 'Student':
      ...
      for sub in check_sub:
        if request.user.student.section == sub.section:
          if sub.inst.user.id == evaluated:
            checked = True
            break
  elif request.user.role == 'Instructor' and request.user.instructor.access_lvl != 3 :
    ...
    for access in check_dept:
      if access.access_lvl == 2:
        checked = True
        break

  if checked == False:
    if request.user.role == 'Student' or (request.user.role == 'Instructor' and request.user.instructor.access_lvl != 3):
      return redirect('/evaluation')
    else:
      return redirect('/evaluation/dean')
...
def view_responses(request):
  ...
  users = User.objects.all()
  eval_deets = EvaluatedDetail.objects.all()
...
def add_bulk_instructor_per_section(request):
  ...
  if request.method == 'POST':
    ...
    check_subjects = Subject.objects.all()
    check_eval_deets = EvaluatedDetail.objects.all()
      ...
      if missing:
        ...
        for data in df.values:
          ...
          for eval in check_eval_deets:
            ...
            if eval.section == section:
              if eval.subj.code == subject:
                skip = False
                break
              elif instructor_check == instructor:
                skip = False
                break
          ...
          evaluated_deets = EvaluatedDetail(
            section = section,
            inst_id = instructor,
            subj_id = subject,
          )
...
def edit_instructor(request):
  ...
  if check_evalsched:
    ...
    if datetime.date.today() >= check_evalsched.date_from and datetime.date.today() <= check_evalsched.date_to:
      return redirect('/evaluation')
  sort_sec = sort_section()
  inst_per_sec = EvaluatedDetail.objects.all()

  return render(
    request,
    'main/edit_instructor.html',
    {'section': sort_sec,
     'instructors': inst_per_sec}
  )
...
def edit_instructor_per_id(request, section, id):
  ...
  if check_evalsched:
    ...
    if datetime.date.today() >= check_evalsched.date_from and datetime.date.today() <= check_evalsched.date_to:
      return redirect('/evaluation')
  instructor = EvaluatedDetail.objects.all()
  instructors = Instructor.objects.all()
  ...
  if request.method == 'POST':
    get_instructor = EvaluatedDetail.objects.get(id=inst.id)
    ...
    get_instructor.save()
    return redirect('edit_instructor')
...
def delete_instructor_per_id(request, id, section):
  ...
  if check_evalsched:
    ...
    if datetime.date.today() >= check_evalsched.date_from and datetime.date.today() <= check_evalsched.date_to:
      return redirect('/evaluation')
  intructor_per_sub = EvaluatedDetail.objects.get(id=id)
...
def add_instructor(request, section):
  ...
  if 'IT-1' in section:
    crs = 'IT'
    yr = 'First'
  elif 'IT-2' in section:
    crs = 'IT'
    yr = 'Second'
  elif 'IT-3' in section:
    crs = 'IT'
    yr = 'Third'
  elif 'IT-4' in section:
    crs = 'IT'
    yr = 'Fourth'
  elif 'IS-1' in section:
    crs = 'IS'
    yr = 'First'
  elif 'IS-2' in section:
    crs = 'IS'
    yr = 'Second'
  elif 'IS-3' in section:
    crs = 'IS'
    yr = 'Third'
  elif 'IS-4' in section:
    crs = 'IS'
    yr = 'Fourth'
  elif 'CS-1' in section:
    crs = 'CS'
    yr = 'First'
  elif 'CS-2' in section:
    crs = 'CS'
    yr = 'Second'
  elif 'CS-3' in section:
    crs = 'CS'
    yr = 'Third'
  elif 'CS-4' in section:
    crs = 'CS'
    yr = 'Fourth'
  elif 'ACT-1' in section:
    crs = 'ACT'
    yr = 'First'
  elif 'ACT-2' in section:
    crs = 'ACT'
    yr = 'Second'
  
  for subject in subjects:
    if subject.course == crs and subject.year == yr:
      subject_choices.append(subject)

  if request.method == "POST":
    add_instructor_per_sub = EvaluatedDetail(
      section = section,
    ...
    skip = False
    check_evalsched = EvaluatedDetail.objects.filter(subj_id=int(request.POST['subject']))
    if check_evalsched:
      for check in check_evalsched:
        if check.section == section:
          if check.subj.id == int(request.POST['subject']):
            skip = True

    if skip == True:
      messages.warning(request, 'Subject is already added to the section')
      return redirect('add_instructor', section=section) 
    else:
...
def edit_subjects(request, id):
  ...
  if request.method == 'POST':
    get_subject = Subject.objects.get(id=subjects.id)
    get_subject.description = request.POST['description']
    get_subject.code = request.POST['code']
    get_subject.course = request.POST['course']
    get_subject.year = request.POST['year']
    get_subject.semester = request.POST['semester']

    check_sub = Subject.objects.all()
    for sub in check_sub:
      if get_subject.id != sub.id:
        if sub.code == request.POST['code'] and sub.course == request.POST['course']:
          messages.warning(request, 'Similar subject code with an existing one')
          return redirect('edit_subjects', id=id)
...
@login_required(login_url="/login")
@authenticated_user_admin
def add_subjects_single(request, crs):
  check_evalsched = EvalSched.objects.all()
  if check_evalsched:
    check_evalsched = EvalSched.objects.all()[0]
    if datetime.date.today() >= check_evalsched.date_from and datetime.date.today() <= check_evalsched.date_to:
  ...
  if request.method == 'POST':
    ...
    for sub in check_sub:
      if sub.code == request.POST['code'] and sub.course == request.POST['course']:
        messages.warning(request, 'Similar subject code with an existing one')
        return redirect('add_subjects_single')
...
def add_subjects_bulk(request):
  ...
  if request.method == 'POST':
    ...
    if 'description' and 'code' and 'course' and 'year' and 'semester' in df.columns:
      missing = df.isnull().values.any()
      if missing:
        messages.warning(request, 'Some values are missing from the file.')
        return redirect('add_subjects_bulk')
        ...
        for data in df.values:
          ...
          if course == 'IT' or course == 'IS' or course == 'CS' or course == 'ACT':
            if year == 'First' or year == 'Second' or year == 'Third' or year == 'Fourth':
              if semester == 'First' or semester == 'Second':
                pass
              else:
                failed.append('Double check the semester you inputted')
                continue
            else:
              failed.append('Double check the year you inputted')
              continue
          else:
            failed.append('Double check the course you inputted')
            continue

          check_sub = Subject.objects.filter(code=code)

          skip = False
          if len(check_sub) != 0:
            for check in check_sub:
              if check.course == course and check.code == code:
                skip = True

          if skip == True:
            subject_found.append(description+'/'+course)
            continue

          subjects_added = Subject(
            code = code,
